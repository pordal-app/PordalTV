...
| R6-1 | 6 | build | `scripts/release_beta.py` | flavor-aware release: `app:assembleFullRelease`, `apk/full/release/`, `app-full-*` asset names (upstream script predates flavors — genuine bug) | Keep; candidate to upstream |
| R6-2 | 6 | build | `app/build.gradle.kts` | `FEATURE_IN_APP_UPDATES_ENABLED` (full flavor) back to `true` — closes R1-3 now that a Pordal release is imminent | Set `false` to disable the GitHub-releases updater |
